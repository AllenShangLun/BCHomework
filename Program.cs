...
namespace CSharpExam
{
            ...
            List<Student> studentList = new List<Student>()
  {
    ...
    new Student() { StudentId = "S0007", Name = "小玉", Gender = GenderOption.Female, CourseList = new List<string>() { "A005", "B002", "B003", "B004" } },
            ...
            {
                var question1 = courseList.GroupBy(x => x.Name).Select(x => x.Key);
                Console.WriteLine(string.Join(",", question1));
            }
            ...
            {
                var question2 = courseList.Where(x => x.Classroom == "L107").Select(x => x.CourseId);
                Console.WriteLine(string.Join(",", question2));
            }
            ...
            {
                var question3 = courseList.Where(x => x.Classroom == "L107" && x.Credit == 3).Select(x => x.CourseId);
                Console.WriteLine(string.Join(",", question3));
            }
            ...
            {
                var question4 = courseList.GroupBy(x => x.Teacher).Select(s => new
                {
                    Teacher = s.Key
                }).Select(x => x.Teacher);
                Console.WriteLine(string.Join(",", question4));
            }
            ...
            {
                var question5 = courseList.Where(x => x.Classroom == "L202").GroupBy(x => x.Teacher).Select(s => new
                {
                    Teacher = s.Key
                }).Select(x => x.Teacher);
                Console.WriteLine(string.Join(",", question5));
            }
            ...
            {
                var question6 = studentList.Where(x => x.Gender == GenderOption.Male).Select(x => x.Name);
                Console.WriteLine(string.Join(",", question6));
            }
            ...
            {
                var getGitIds = courseList.Where(course => course.Name == "Git")
                              .Select(course => course.CourseId);
                var question7 = studentList.Where(student => student.CourseList.Any(courseId => getGitIds.Contains(courseId)))
                                 .Select(student => student.Name);

                Console.WriteLine(string.Join(",", question7));
            }
            ...
            {
                var question8 = studentList.Select(s => new
                {
                    StudentName = s.Name,
                    Courses = s.CourseList.Select(courseId => courseList.First(course => course.CourseId == courseId).Name)
                }).Select(student => $"學員: {student.StudentName}\n課程: {string.Join(", ", student.Courses)}").ToList();

                Console.WriteLine(string.Join(Environment.NewLine + Environment.NewLine, question8));
            }
            ...
            {
                var question9 = studentList.OrderBy(x => x.CourseList.Count).First();
                Console.WriteLine(question9.Name);
            }
            ...
            {
                var question10 = studentList.Select(s => new
                {
                    s.Name,
                    SumCredit = s.CourseList.Join(courseList, student => student, c => c.CourseId, (student, c) => c.Credit).Sum()
                }).Where(x => x.SumCredit < 10).Select(x=>x.Name);
                Console.WriteLine(string.Join (",", question10));
            }
            ...
            {
                var question11 = studentList.Select(s => new
                {
                    s.Name,
                    SumCredit = s.CourseList.Join(courseList, student => student, c => c.CourseId, (student, c) => c.Credit).Sum()
                }).OrderByDescending(x => x.SumCredit).First();
                Console.WriteLine(string.Join(",", question11.Name));
            }
            ...
            Console.WriteLine("12. 十二生肖自定義排序");
            {
                ...
                Console.Write("排序後: ");
                //作答區
                var zooOrderBy = new List<string> { "鼠", "牛", "虎", "兔", "龍", "蛇", "馬", "羊", "猴", "雞", "狗", "豬" };
                zoo.Sort((x, y) => zooOrderBy.IndexOf(x).CompareTo(zooOrderBy.IndexOf(y)));
